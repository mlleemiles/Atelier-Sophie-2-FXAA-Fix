...
cbuffer _Globals : register(b0)
{
  ...
  float fStarWeight : packoffset(c2) = {0.800000012};
  float fLensFlareWeight : packoffset(c2.y) = {0.300000012};
  float2 SimulateHDRParams : packoffset(c2.z);
  float4 vLightShaftPower : packoffset(c3);
...
SamplerState smplStar : register(s8);
SamplerState smplFlare : register(s9);
SamplerState smplLightShaftLinWork2 : register(s10);
...
Texture2D<float4> smplStar_Tex : register(t8);
  ...
  out float4 o0 : SV_Target0)
{
  ...
  r0.xyz = smplStar_Tex.Sample(smplStar, v1.xy).xyz;
  r0.xyz = r0.xyz * fStarWeight + r2.yzw;
